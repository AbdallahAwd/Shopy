...
class HamadaScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Builder(
        builder: (context) {
          ReqCubit.get(context).listenHamadaData();
            ...
            builder: (context, state) {
              return Scaffold(
                ...
                body: ListView.separated(
                    itemBuilder: (context , index) => eldawliaCheckBox(context , ReqCubit.get(context).hamada[index] , index),
                    separatorBuilder: (context , index) =>const SizedBox(height: 10,child: Divider(height: 1,),),
                    itemCount:  ReqCubit.get(context).hamada.length
                ),
  ...
  showAlertDialog(context) {
    ...
    Widget okButton = TextButton(
      ...
      onPressed: () {
        ReqCubit.get(context).addHamadaReq(
    ...
    AlertDialog alert = AlertDialog(
      ...
      title:  Center(child: Text("${getLang(context, 'reqProduct')}" , style: Theme.of(context).textTheme.bodyText1.copyWith(fontFamily: 'Cairo'),)),
      content: Column(
  ...
  Widget eldawliaCheckBox(context , ReqModel model ,index) =>
      GestureDetector(
        onLongPress: ()
        {
          ...
          ReqCubit.get(context).deleteHamadaItem(ReqCubit.get(context).hamadaId[index]);
        },
        child: CheckboxListTile(
          title: Text('${model.product} ${getLang(context , 'Q is')} ${model.quantity}', style: Theme
              ...
              .get(context)
              .hamada[index].checkedValues ? TextDecoration.lineThrough : null , fontFamily: 'Cairo' ),),
          value: ReqCubit
              .get(context)
              .hamada[index].checkedValues,
          ...
          tileColor:HomeCubit.get(context).isDark ? HexColor('#24252b') : Colors.white ,
          onChanged: (newValue)
          {
            ReqCubit.get(context).updataHamada(quantity: model.quantity , product: model.product , newValue: newValue , hamadaId: ReqCubit.get(context).hamadaId[index]);
          },
