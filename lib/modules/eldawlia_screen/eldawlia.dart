...
class ElDawliaScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Builder(
      builder: (context) {
        ReqCubit.get(context).listenData();
          ...
          builder: (context, state) {
            return Scaffold(
                ...
                body: ListView.separated(
                    itemBuilder: (context , index) => eldawliaCheckBox(context , ReqCubit.get(context).req[index] , index),
                    separatorBuilder: (context , index) =>const SizedBox(height: 10,child: Divider(height: 1,),),
                    itemCount:  ReqCubit.get(context).req.length
                ),
  ...
  showAlertDialog(context) {
    ...
    Widget okButton = TextButton(
      ...
      onPressed: () {
        ReqCubit.get(context).addEldawliaItem(
  ...
  Widget eldawliaCheckBox(context , ReqModel model ,index) =>
      GestureDetector(
        onLongPress: ()
        {
          ...
          ReqCubit.get(context).delete(ReqCubit.get(context).reqId[index]);
        },
        child: CheckboxListTile(
          ...
          title: Text('${model.product} ${getLang(context , 'Q is')} ${model.quantity}', style: Theme
              ...
              .get(context)
              .req[index].checkedValues ? TextDecoration.lineThrough : null , fontFamily: 'Cairo' ),),
          value: ReqCubit
              .get(context)
              .req[index].checkedValues,
          ...
          onChanged: (newValue)
          {
            ReqCubit.get(context).updateList(ReqCubit.get(context).reqId[index], newValue ,  model.product , model.quantity);
          },
