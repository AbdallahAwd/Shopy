...
class SawahScreen extends StatelessWidget
{
  ...
  Widget build(BuildContext context) {
    return Builder(
        builder: (context) {
          ReqCubit.get(context).ListenSawah();
            ...
            builder: (context, state) {
              return Scaffold(
                ...
                body: ListView.separated(
                    itemBuilder: (context , index) => eldawliaCheckBox(context , ReqCubit.get(context).sawah[index] , index),
                    separatorBuilder: (context , index) =>const SizedBox(height: 10,child: Divider(height: 1,),),
                    itemCount:  ReqCubit.get(context).sawah.length
                ),
  ...
  Widget eldawliaCheckBox(context , ReqModel model ,index) =>
      GestureDetector(
        onLongPress: ()
        {
          ...
          ReqCubit.get(context).deleteSawah(ReqCubit.get(context).sawahId[index]);
        },
        child: CheckboxListTile(
          title: Text('${model.product} ${getLang(context , 'Q is')} ${model.quantity}', style: Theme
              ...
              .get(context)
              .sawah[index].checkedValues ? TextDecoration.lineThrough : null , fontFamily: 'Cairo' ),),
          value: ReqCubit
              .get(context)
              .sawah[index].checkedValues,
          ...
          {
            ReqCubit.get(context).updateSawah(quantity: model.quantity , product: model.product , newValue: newValue , sawahId: ReqCubit.get(context).sawahId[index]);
          },
