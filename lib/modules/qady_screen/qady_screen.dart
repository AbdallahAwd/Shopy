...
class QadyScreen extends StatelessWidget
{
  ...
  Widget build(BuildContext context) {
    return Builder(
        builder: (context) {
          ReqCubit.get(context).ListenQady();
            ...
            builder: (context, state) {
              return Scaffold(
                ...
                body: ListView.separated(
                    itemBuilder: (context , index) => eldawliaCheckBox(context , ReqCubit.get(context).qady[index] , index),
                    separatorBuilder: (context , index) =>const SizedBox(height: 10,child: Divider(height: 1,),),
                    itemCount:  ReqCubit.get(context).qady.length
                ),
  ...
  Widget eldawliaCheckBox(context , ReqModel model ,index) =>
      GestureDetector(
        onLongPress: ()
        {
          ...
          ReqCubit.get(context).deleteQady(ReqCubit.get(context).qadyId[index]);
        },
        child: CheckboxListTile(
          title: Text('${model.product} ${getLang(context , 'Q is')} ${model.quantity}', style: Theme
              ...
              .get(context)
              .qady[index].checkedValues ? TextDecoration.lineThrough : null , fontFamily: 'Cairo' ),),
          value: ReqCubit
              .get(context)
              .qady[index].checkedValues,
          ...
          {
            ReqCubit.get(context).updateQady(quantity: model.quantity , product: model.product , newValue: newValue , qadyId: ReqCubit.get(context).qadyId[index]);
          },
